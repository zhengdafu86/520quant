"""
见顶防御早警验证 — 大盘连续N日<MA20(可选+缩量)强制停新建仓, 救2024见顶/不伤2025-26
对照: off(现行) / N3 / N5 / N5+缩量。两段都跑。
用法: BT_DAILY_BARS=700 python3 -B -m backtest.validate_topwarn
"""
...
MODES = [("现行(off)", 0, False), ("N3日<MA20", 3, False),
         ("N5日<MA20", 5, False), ("N5+缩量", 5, True)]
...
def main():
    ...
    for plab, s, e in PERIODS:
        ...
        print(f"\n── {plab} ({s}~{e}, 5m {len(m5_map)}只)")
        print(f"   {'方案':<14}{'收益%':>8}{'回撤%':>7}{'卡玛':>7}{'Alpha%':>8}{'交易':>6}{'胜率%':>7}")
        for nm, nd, vol in MODES:
            t, ec, pos = simulate(ctx, names, s, e, max_pos=4, capital=CAP, slippage=0.001,
                                  ...
                                  topwarn_days=nd, topwarn_vol=vol)
    ...
    print("\n判读: 2024收益↑/回撤↓(救见顶) 且 2025-26不掉(不伤好年) ⇒ 早警有用; 2025-26掉 ⇒ 误杀健康回调。")
