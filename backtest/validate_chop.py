"""
震荡股进场过滤 — 效率系数ER / MA20穿越次数 做进场闸, 防whipsaw
两段都跑: 2024(whipsaw年,73%止损) 能否救 + 2025-26(趋势年) 不伤。
用法: BT_DAILY_BARS=700 python3 -B -m backtest.validate_chop
"""
...
# (标签, er_min, cross_max)
MODES = [("现行(off)", 0.0, 0),
         ("ER≥0.2", 0.2, 0), ("ER≥0.3", 0.3, 0), ("ER≥0.4", 0.4, 0),
         ("穿越≤6", 0.0, 6), ("穿越≤4", 0.0, 4), ("穿越≤3", 0.0, 3)]
...
def main():
    ...
    for plab, s, e in PERIODS:
        ...
        print(f"\n── {plab} ({s}~{e})")
        print(f"   {'闸':<12}{'收益%':>8}{'回撤%':>7}{'卡玛':>7}{'Alpha%':>8}{'交易':>6}{'胜率%':>7}")
        for nm, er, cx in MODES:
    ...
    print("\n判读: 2024收益↑/回撤↓/胜率↑(滤掉震荡股) 且 2025-26不掉 ⇒ 闸有效; 2025-26掉 ⇒ 误杀趋势股回踩。")
